...
interface KafkaTopicFormProps {
  hideHeader?: boolean;
}

export const KafkaTopicForm: React.FC<KafkaTopicFormProps> = ({ hideHeader = false }) => {
  ...
  const [request, setRequest] = useState<OnboardRequest>({
    requestCriteria: ['kafkatopic'], // Only Kafka topic
    downstreamDetails: [],
  });
  ...
  const mutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      if (failureCount === 0 && partialCount === 0) {
        setOperationStatus({
          type: 'success',
          message: `Kafka topic creation completed successfully! All ${tasks.length} task(s) completed successfully in ${environment} environment.`,
        });
      } else if (failureCount > 0) {
        setOperationStatus({
          type: 'error',
          message: `Kafka topic creation completed with errors. ${failureCount} task(s) failed, ${successCount} succeeded in ${environment} environment. Please review the details below.`,
        });
      } else if (partialCount > 0) {
        setOperationStatus({
          type: 'warning',
          message: `Kafka topic creation completed with partial success. ${partialCount} task(s) completed partially, ${successCount} succeeded in ${environment} environment. Please review the details below.`,
        });
      }
    },
    onError: (error: Error) => {
      ...
      const errorResult = {
        task: 'Kafka Topic Creation Error',
      ...
      setOperationStatus({
        type: 'error',
        message: `Kafka topic creation failed: ${errorMessage}. Please check your request and try again.`,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const backendEnvironment = getBackendEnvironment(environment);
    if (!backendEnvironment) {
      setValidationErrors({ ...validationErrors, environment: 'Invalid environment selected' });
      return;
    }
      ...
      {!hideHeader && (
        <div className="mb-8">
          ...
          <div className="bg-gradient-to-r from-primary-500 via-primary-400 to-primary-500 rounded-xl shadow-2xl p-6 mb-4 border-2 border-primary-600">
            <h1 className="text-2xl font-bold text-white mb-2 drop-shadow-lg">Create New Kafka Topic</h1>
            ...
            <div className="bg-white rounded-xl shadow-2xl border-2 border-primary-400 p-6">
              ...
              <div className="mb-4">
                ...
                <input
                  ...
                  onChange={(e) => updateRequest({ numPartitions: parseInt(e.target.value) || undefined })}
                  className={`w-full px-4 py-2.5 border-2 rounded-lg focus:outline-none focus:ring-2 transition-all bg-white text-gray-900 placeholder-gray-400 ${
                    validationErrors.numPartitions 
                      ? 'border-red-500 focus:ring-red-300 focus:border-red-500' 
                      : 'border-primary-400 focus:ring-primary-300 focus:border-primary-500'
                  }`}
                  placeholder="Enter number of partitions (minimum 1)"
                />
                {validationErrors.numPartitions && (
                  <p className="mt-1 text-sm text-red-600 font-medium">
                    {validationErrors.numPartitions}
              ...
              <div className="mb-4">
                ...
                <input
                  ...
                  onChange={(e) => updateRequest({ replicationFactor: parseInt(e.target.value) || undefined })}
                  className={`w-full px-4 py-2.5 border-2 rounded-lg focus:outline-none focus:ring-2 transition-all bg-white text-gray-900 placeholder-gray-400 ${
                    validationErrors.replicationFactor 
                      ? 'border-red-500 focus:ring-red-300 focus:border-red-500' 
                      : 'border-primary-400 focus:ring-primary-300 focus:border-primary-500'
                  }`}
                  placeholder="Enter replication factor (minimum 1)"
                />
                {validationErrors.replicationFactor && (
                  <p className="mt-1 text-sm text-red-600 font-medium">
                    {validationErrors.replicationFactor}
